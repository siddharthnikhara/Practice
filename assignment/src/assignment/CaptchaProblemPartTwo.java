...
public class CaptchaProblemPartTwo {

	public static void problemPartTwo(Integer[][] array){
		...
		l1:	for(int i=0;i<array.length;i++){
			...
			l2:	for(int j=0;j<array[0].length;j++){
				...
				//	Integer division=(list.get(k))%(list.get(p))==0;
				//	if(list.get(k)%)
					
					if(p!=k) {
					if((list.get(k)%list.get(p))==0) {
						System.out.println(list.get(k)+" "+list.get(p));
						Integer division=(list.get(k)/list.get(p));
						list1.add(division);
